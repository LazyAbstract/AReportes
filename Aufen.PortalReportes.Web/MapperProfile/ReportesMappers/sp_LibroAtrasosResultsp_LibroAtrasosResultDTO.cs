    ...
    public class sp_LibroAtrasosResultsp_LibroAtrasosResultDTO : Profile
    {
        ...
        protected override void Configure()
        {
            ...
            Mapper.CreateMap<sp_LibroAsistenciaResult, LibroAsistenciaDTO>()
                ...
                .ForMember(x => x.Entrada, prop => prop.MapFrom(x => !String.IsNullOrEmpty(x.Entrada) && !String.IsNullOrWhiteSpace(x.Entrada) ? DateTime.ParseExact(x.Fecha + (x.Entrada.Substring(0,4)+"00"), "yyyyMMddHHmmss", CultureInfo.CurrentCulture) : nulo))
                .ForMember(x => x.EntradaColacion, prop => prop.MapFrom(x => !String.IsNullOrEmpty(x.EntradaColacion) && !String.IsNullOrWhiteSpace(x.EntradaColacion) ? DateTime.ParseExact(x.Fecha + x.EntradaColacion.Substring(0,4)+"00", "yyyyMMddHHmmss", CultureInfo.CurrentCulture) : nulo))
                .ForMember(x => x.Salida, prop => prop.MapFrom(x => !String.IsNullOrEmpty(x.Salida) && !String.IsNullOrWhiteSpace(x.Salida) ? DateTime.ParseExact(x.Fecha + x.Salida.Substring(0, 4) + "00", "yyyyMMddHHmmss", CultureInfo.CurrentCulture) : nulo))
